      ...
      <div className="bg-white w-full max-w-5xl rounded-[2.5rem] shadow-2xl border border-border-ghost overflow-hidden animate-in zoom-in-95 duration-300 flex flex-col max-h-[90vh]">
        <header className="px-8 py-6 bg-white border-b border-border-ghost sticky top-0 z-20 flex items-start justify-between shrink-0">
          <div className="flex gap-6">
            ...
            <div>
              <nav className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.2em] text-muted-foreground mb-1.5">
                  ...
                  return (
                    <>
                      <div className="flex items-center gap-2 px-3 py-1.5 bg-blue-400 border border-blue-500/50 text-blue-950 rounded-xl shadow-sm">
                        ...
                        <span className="text-xs font-black">+{pipelineRemaining}</span>
                      </div>
                      ...
                      {reservedQty > 0 && (
                        <div className="flex items-center gap-2 px-3 py-1.5 bg-yellow-400 border border-yellow-500/50 text-yellow-900 rounded-xl shadow-sm">
                          ...
                          <span className="text-xs font-black">+{reservedQty}</span>
                        </div>
                      )}
                      ...
                      <div className="flex items-center gap-2 px-3 py-1.5 bg-surface-low rounded-xl border border-border-ghost shadow-sm">
                        <span className="text-xs font-black opacity-40 uppercase tracking-widest">Latest</span>
                        <span className="text-xs font-black text-foreground">
                      ...
                      <div className="flex items-center gap-2 px-3 py-1.5 bg-surface-low rounded-xl border border-border-ghost shadow-sm">
                        <span className="text-xs font-black opacity-40 uppercase tracking-widest">Min Req</span>
                        <span className="text-xs font-black text-foreground">
                          {minStockLevel}
                        </span>
                      </div>
            ...
            <div className="space-y-8">
              ...
              <div className="flex items-center gap-1.5 bg-surface-low/30 p-1 rounded-2xl border border-border-ghost w-fit mx-auto sm:mx-0">
                <button
                  ...
                  className={cn(
                    "flex items-center gap-2.5 px-8 py-2.5 rounded-xl font-black text-xs uppercase tracking-widest transition-all",
                    activeTab === "buying"
                              ...
                              {breakdown.length > 0 ? breakdown.map((entry, idx) => (
                                <tr key={idx} className="group hover:bg-white/50 transition-colors">
                                  ...
                                  <td className="table-cell !py-3">
                                    <div className="flex flex-col">
                                      ...
                                      <span className="text-xs font-black text-muted-foreground uppercase tracking-widest mt-0.5">{new Date(entry.purchaseDate).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: true })}</span>
                                    </div>
                                  </td>
                                  <td className="table-cell !py-3">
                                ...
                                <tr>
                                  <td colSpan={5} className="py-10 text-center">
                                    ...
                                    <p className="text-xs font-bold text-muted-foreground italic tracking-tight">No active stock batches found.</p>
                                  </td>
                                </tr>
                              )}
                            </tbody>
                                  ...
                                  <td className="table-cell !py-3">
                                    <div className="flex items-center gap-3">
                                      ...
                                      <div className="flex flex-col">
                                        ...
                                        <span className="text-xs font-black text-primary uppercase tracking-widest mt-0.5">PO #{po.poId.split("-")[0].toUpperCase()}</span>
                                      </div>
                                    </div>
                                  </td>
                                  <td className="table-cell !py-3">
                                    <div className="flex flex-col">
                                      ...
                                      <span className="text-xs font-black text-emerald-600 uppercase tracking-widest mt-0.5">{po.status}</span>
                                    </div>
                                  </td>
                                  <td className="table-cell !py-3 text-right">
                  ...
                  <div className="space-y-8">
                                ...
                                <tr key={idx} className="group hover:bg-white/50 transition-colors">
                                  ...
                                  <td className="table-cell !py-3">
                                    <div className="flex flex-col">
                                      ...
                                      <span className="text-xs font-black text-muted-foreground uppercase tracking-widest mt-0.5">{new Date(entry.date).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: true })}</span>
                                    </div>
                                  </td>
                                  <td className="table-cell !py-3 text-right">
                    ...
                    <section className="space-y-6">
                      <div className="flex items-center justify-between mb-4 px-1">
                        <h4 className="text-xs font-black text-muted-foreground uppercase tracking-[0.2em]">Pending Customer Deliveries</h4>
                              ...
                              {customerOrders.length > 0 ? customerOrders.map((order) => (
                                <tr key={order.orderId} className="group hover:bg-white/50 transition-colors">
                                  <td className="table-cell !py-3">
                                    <div className="flex items-center gap-3">
                                      ...
                                      <div className="flex flex-col">
                                        ...
                                        <span className="text-xs font-black text-primary uppercase tracking-widest mt-0.5">ORDER #{order.orderId.split("-")[0].toUpperCase()}</span>
                                      </div>
                                    </div>
                                  </td>
                                  <td className="table-cell !py-3">
                                  ...
                                  <td className="table-cell !py-3 text-center">
                                    <span className={`badge !text-xs !px-2 !py-0.5 ${order.status === 'pending' ? 'badge-warning' : 'badge-primary'}`}>
                                      {order.status}
                                    </span>
                                  </td>
                                </tr>
                                ...
                                <tr>
                                  <td colSpan={5} className="py-10 text-center">
                                    ...
                                    <p className="text-xs font-bold text-muted-foreground italic tracking-tight">No pending customer deliveries.</p>
                                  </td>
                                </tr>
                              )}
                            </tbody>
        ...
        <footer className="px-8 py-6 bg-white border-t border-border-ghost shrink-0 flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-muted-foreground mt-0.5 italic leading-none">Real-time inventory ledger active</p>
            </div>
          </div>
          <button
            onClick={onClose}

...
export function ItemBreakdownModal({ 
  ...
  const [activeTab, setActiveTab] = useState<"buying" | "selling" | "incoming" | "pending_selling">("buying");
             ...
             <div className="space-y-12">
                ...
                <div className="flex items-center gap-1.5 bg-surface-low/30 p-1.5 rounded-3xl border border-border-ghost w-fit mx-auto sm:mx-0">
                   <button 
                     ...
                     className={`flex items-center gap-3 px-8 py-3.5 rounded-[1.25rem] font-black text-xs uppercase tracking-widest transition-all ${
                       activeTab === "buying" 
                   ...
                   <div className="w-px h-8 bg-border-ghost mx-2 hidden sm:block opacity-50"></div>

                   <button 
                     onClick={() => setActiveTab("pending_selling")}
                     className={`flex items-center gap-3 px-8 py-3.5 rounded-[1.25rem] font-black text-xs uppercase tracking-widest transition-all ${
                       activeTab === "pending_selling" 
                         ? "bg-white text-indigo-600 shadow-lg shadow-indigo-600/5 border border-border-ghost" 
                         : "text-muted-foreground hover:text-foreground hover:bg-white/50"
                     }`}
                   >
                     <ShoppingCart className={cn("w-4 h-4 transition-transform", activeTab === "pending_selling" && "scale-110")} />
                     <span>Orders</span>
                     {customerOrders.length > 0 && (
                       <span className={`px-2 py-0.5 rounded-md text-[9px] ${activeTab === "pending_selling" ? "bg-indigo-600/10 text-indigo-600" : "bg-muted-foreground/10 text-muted-foreground"}`}>
                   ...
                   <button 
                     ...
                     className={`flex items-center gap-3 px-8 py-3.5 rounded-[1.25rem] font-black text-xs uppercase tracking-widest transition-all ${
                       activeTab === "incoming" 
                         ? "bg-white text-emerald-600 shadow-lg shadow-emerald-600/5 border border-border-ghost" 
                         : "text-muted-foreground hover:text-foreground hover:bg-white/50"
                     }`}
                   >
                     <Truck className={cn("w-4 h-4 transition-transform", activeTab === "incoming" && "scale-110")} />
                     <span>Pipeline</span>
                  ...
                  {activeTab === "buying" && (
                    <section className="space-y-8">
                      <div className="flex items-end justify-between border-b border-border-ghost pb-6 px-2">
                        ...
                        <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest opacity-40 bg-surface-low px-3 py-1 rounded-full border border-border-ghost">LATEST 20 RECEIPTS</span>
                      </div>
                      ...
                      <div className="grid grid-cols-1 gap-4">
                        {purchaseHistory.length > 0 ? purchaseHistory.map((entry, idx) => (
                          ...
                          <div className="bg-surface-low/30 rounded-[2.5rem] border border-border-ghost border-dashed py-32 text-center">
                             <ArrowDownLeft className="w-16 h-16 text-muted-foreground opacity-10 mx-auto mb-6" />
                             <p className="text-sm font-bold text-muted-foreground italic tracking-tight">No inward history recorded for this asset.</p>
                          </div>
                        )}
                      </div>
                    </section>
                    ...
                    <section className="space-y-8">
                      ...
                      <div className="grid grid-cols-1 gap-4">
                        {sellingHistory.length > 0 ? sellingHistory.map((entry, idx) => (
                          ...
                          <div className="bg-surface-low/30 rounded-[2.5rem] border border-border-ghost border-dashed py-32 text-center">
                             <ArrowUpRight className="w-16 h-16 text-muted-foreground opacity-10 mx-auto mb-6" />
                             <p className="text-sm font-bold text-muted-foreground italic tracking-tight">No outward history recorded for this asset.</p>
                          </div>
                        )}
                      </div>
                    </section>
                    ...
                    <section className="space-y-8">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {customerOrders.length > 0 ? customerOrders.map((order) => (
                          ...
                          <div className="col-span-full bg-surface-low/30 rounded-[2.5rem] border border-border-ghost border-dashed py-32 text-center">
                             <ShoppingCart className="w-16 h-16 text-muted-foreground opacity-10 mx-auto mb-6" />
                             <p className="text-sm font-bold text-muted-foreground italic tracking-tight">No active customer reservations.</p>
                          </div>
                        )}
                      </div>
                    </section>
                    ...
                    <section className="space-y-8">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {incomingPOs.length > 0 ? incomingPOs.map((po) => (
                          ...
                          <div className="col-span-full bg-surface-low/30 rounded-[2.5rem] border border-border-ghost border-dashed py-32 text-center">
                             <Truck className="w-16 h-16 text-muted-foreground opacity-10 mx-auto mb-6" />
                             <p className="text-sm font-bold text-muted-foreground italic tracking-tight">No incoming shipments scheduled.</p>
                          </div>
                        )}
                      </div>
                    </section>

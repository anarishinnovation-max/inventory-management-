    ...
    <div className="space-y-10 pb-10">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <h1 className="text-4xl font-black text-foreground tracking-tight">Customer Orders</h1>
          <p className="text-muted-foreground mt-2 text-lg font-medium">Manage orders to customers</p>
        </div>
        <Link href="/orders/dispatch/new" className="flex items-center gap-2 px-6 py-3 bg-linear-to-r from-primary to-indigo-600 text-white rounded-xl font-bold shadow-lg shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-surface-lowest p-8 rounded-4xl border border-border-ghost shadow-ambient group hover:border-orange-500/30 transition-all">
           <div className="flex justify-between items-start">
              ...
              <span className="text-xs font-black text-orange-600">READY TO SHIP</span>
           </div>
           <div className="mt-8">
              <h2 className="text-4xl font-black text-foreground tracking-tighter">{pendingCount}</h2>
              <p className="text-sm font-bold text-muted-foreground mt-1">Ready to ship</p>
           </div>
        </div>
        ...
        <div className="bg-surface-lowest p-8 rounded-4xl border border-border-ghost shadow-ambient group hover:border-emerald-500/30 transition-all">
           <div className="flex justify-between items-start">
              ...
              <span className="text-xs font-black text-emerald-600">SHIPPED</span>
           </div>
           <div className="mt-8">
              <h2 className="text-4xl font-black text-foreground tracking-tighter">{dispatchedCount}</h2>
              <p className="text-sm font-bold text-muted-foreground mt-1">Already shipped</p>
           </div>
        </div>
      </div>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-surface-low/30 border-b border-border-ghost">
                      ...
                      <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-muted-foreground">Items</th>
                  ...
                  <tbody className="divide-y divide-border-ghost">
                    {orders.length > 0 ? orders.map((order: any) => (
                      <tr key={order.id} className="hover:bg-surface-low/30 transition-colors group cursor-pointer">
                        ...
                        <td className="px-8 py-6">
                           <span className="text-sm font-bold text-foreground">{order.paymentMode || "Cash"}</span>
                        </td>
                        <td className="px-8 py-6">
                           <span className={cn(
